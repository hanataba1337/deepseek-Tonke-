...
基于 DeepSeek 官方定价（¥/百万 Token），按 **95% 缓存命中率**计算有效输入价格：

| 模型 | 有效输入价 | 输出 | 原始缓存未命中价 |
|------|:---:|:---:|:---:|
| DeepSeek-V4-Flash | **¥0.069** | ¥2.00 | ¥1.00 |
| DeepSeek-V4-Pro | **¥0.174** | ¥6.00 | ¥3.00 |

> 可在 `config.py` 中调整 `CACHE_HIT_RATE`（默认 0.95）和 `PRICING` 字典。
